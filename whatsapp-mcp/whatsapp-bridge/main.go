...
func resolveRecipientJID(client *whatsmeow.Client, recipient string) (types.JID, error) {
	...
	if recipientJID.Server == types.DefaultUserServer {
		...
		if client.Store == nil || client.Store.LIDs == nil {
			// Unpaired or partially initialized store — skip local cache and
			// let the server lookup below decide (it will fail cleanly).
			lidErr = fmt.Errorf("LID store unavailable")
		} else {
...
func newRESTMux(client *whatsmeow.Client, messageStore *MessageStore, port int, token string, allowedMediaRoots []string) *http.ServeMux {
	...
	// Live pairing QR for browser-based device linking (see pairing.go)
	registerPairingEndpoints(mux, auth)

	// Delete-for-everyone (revoke) endpoint (see delete_message.go)
	registerDeleteEndpoint(mux, auth, client, messageStore)

	// WhatsApp VoIP calling endpoint (see call.go)
	registerCallEndpoints(mux, auth, client, messageStore)
	...
	mux.HandleFunc("/api/health", auth(func(w http.ResponseWriter, r *http.Request) {
		...
		status := map[string]interface{}{
			...
			// IsConnected is also true during the pre-pairing socket phase;
			// logged_in reflects an actual paired WhatsApp session.
			"logged_in": loggedIn,
	...
	mux.HandleFunc("/api/send", auth(func(w http.ResponseWriter, r *http.Request) {
		...
		// Fail fast with a clear error when no WhatsApp session exists yet,
		// instead of panicking deeper in the send path (LID store is nil
		...
		if client.Store == nil || client.Store.ID == nil {
			...
			_ = json.NewEncoder(w).Encode(SendMessageResponse{
				Success: false,
				Message: "Not logged in — scan the QR code to pair WhatsApp first",
			})
			return
		}
	...
	// GET /api/avatar?jid=...&refresh=1 — serve a contact/group profile photo.
	// Served from a local disk cache when present; otherwise fetched via
	// whatsmeow and cached under store/avatars/.
	mux.HandleFunc("/api/avatar", auth(func(w http.ResponseWriter, r *http.Request) {
		...
		safeName := strings.ReplaceAll(jid.String(), ".", "_")
		cacheDir := filepath.Join("store", "avatars")
		if err := os.MkdirAll(cacheDir, 0o755); err != nil {
			http.Error(w, "cache dir failed", http.StatusInternalServerError)
			return
		}
		cacheFile := filepath.Join(cacheDir, safeName+".jpg")
		if !refresh {
		...
		if err != nil || pic == nil || pic.URL == "" {
			noPhoto := errors.Is(err, whatsmeow.ErrProfilePictureNotSet) ||
				errors.Is(err, whatsmeow.ErrProfilePictureUnauthorized)
			status := http.StatusBadGateway
			msg := "profile picture lookup failed"
			if err != nil {
				msg = err.Error()
			}
			if noPhoto {
		...
		imgResp, err := http.Get(pic.URL)
		if err != nil {
			http.Error(w, "profile picture fetch failed", http.StatusBadGateway)
			return
		}
		...
		if err != nil || len(data) == 0 {
			http.Error(w, "profile picture read failed", http.StatusBadGateway)
			return
		}
...
func main() {
	...
	// Start the REST server before pairing so orchestrators can reach
	// /api/health while WhatsApp is unpaired (it reports connected=false,
	// HTTP 503, until a session exists). Mutating endpoints fail fast with
	// clear errors until pairing completes.
	...
	for attempt := 1; attempt <= maxRetries; attempt++ {
		...
		if client.Store.ID == nil {
			...
			// Print QR code for pairing with phone. Every rotation is
			// recorded for /api/qr.png (browser pairing) and printed so
			// terminal viewers always see the current code.
			for evt := range qrChan {
				if evt.Event == "code" {
					storeLatestQR(evt.Code)
					fmt.Println("\nScan this QR code with your WhatsApp app (rotates every ~1 minute):")
...
connectionSuccess:
	...
	// pairing loop, so the setup banner could print immediately. The REST
	// server itself also started before pairing (see above) so health checks
	// work while unpaired.
