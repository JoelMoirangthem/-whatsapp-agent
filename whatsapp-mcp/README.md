...
### Deleting a message (revoke)

`DELETE /api/messages` revokes a previously sent message — WhatsApp's
"delete for everyone". `POST` is also accepted for HTTP clients that cannot
send a body with `DELETE`.

```bash
curl -X DELETE http://127.0.0.1:8080/api/messages \
  -H "Authorization: Bearer $(cat whatsapp-bridge/store/.bridge-token)" \
  -H "Content-Type: application/json" \
  -d '{"chat_jid": "1234567890@s.whatsapp.net", "message_id": "3EB0C50B1BE2E9A1"}'
```

| Field | Required | Description |
| --------- | -------- | --------------------------------------------------------- |
| `chat_jid` | yes | Chat containing the message (`...@s.whatsapp.net`, `...@g.us`, or `...@lid`) |
| `message_id` | yes | ID of the message to revoke |
| `sender_jid` | no | Original sender's JID — required only when deleting another participant's message (group admin revoke); must match the recorded author |

Safety model (fail-closed):

- **The message must exist in the local store.** Unknown IDs return `404` and
  nothing is sent to WhatsApp, so ownership is always verified first.
- **Only your own messages can be deleted in direct chats.** Revoking another
  person's DM messages is not possible on WhatsApp and returns `403`.
- **Group admin revokes require `sender_jid`.** It must match the author
  recorded in the local store (`403` otherwise). LID↔phone forms are treated
  as equivalent when the LID mapping knows both.
- On success the local row is stamped `deleted_at` immediately; the later
  REVOKE echo from WhatsApp deduplicates via first-revoke-wins.
- Content stays in the local archive — the row is marked retracted, not erased,
  mirroring how inbound revokes are handled in `messages.db`.
- WHATSAPP's server enforces its own time window for "delete for everyone"
  (roughly two days for your own messages). Set
  `WHATSAPP_DELETE_MAX_AGE_HOURS` to additionally reject stale revokes locally
  with a clear error instead of a server-side failure.
